function Add-AadGroupMemberIDS {
...
function Add-CannotDeleteLockIDS {
  ...
  Invoke-AzureApiIDS -Method PUT -uri $Url -body $body
  Write-LoggerIDS -type "info" -message "A CanNotDelete lock is being added for $resourceName"
}

function Add-RoleAssignmentIDS {
...
function ConvertFrom-TransportStringIDS {
  ...
  $decodedTransportString = [System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String($transportString))
  $transportObj = $decodedTransportString | ConvertFrom-Json
  
  return $transportObj
}

function ConvertTo-TransportStringIDS {
  # convert a Powershell object to a string that can be passed between pipeline tasks
  param (
    [Parameter()][PSCustomObject]$transportObj
  )

  $jsonString = $transportObj | ConvertTo-Json
  $jsonBytes = [System.Text.Encoding]::Unicode.GetBytes($jsonString)
  $transportString =[Convert]::ToBase64String($jsonBytes)

  return $transportString
}

function Find-AzResourceIDS {
  param (
  ...
  $subscriptionIds = @()
  $resources = @{}
  $token = Get-CachedTokenIDS
  $apiUrl = "https://management.azure.com/subscriptions?api-version=2020-01-01"

  $subs = (Invoke-AzureApiIDS -method Get -Uri $apiUrl).value
  ...
  foreach ($resourceName in $resourceNames) {
    if ($resourceName) {
      ...
      foreach ($subscriptionId in $subscriptionIds) {
        ...
        $resource = Invoke-AzureApiIDS -method Get -Uri $apiUrl
...
function Get-BearerTokenIDS {
    ...
    try {
        ...
        $response = $_
        Write-LoggerIDS -type "warning" -message "StatusCode: $($response.Exception.Response.StatusCode.value)"
        Write-LoggerIDS -type "warning" -message "StatusDescription: $($response.Exception.Response.StatusDescription)"
    }

  return  ('Bearer {0}' -f ($authToken.access_token))
}

function Get-CachedTokenIDS {
...
function Get-PrincipalIDS {
  ...
  try {
    ...
    $authToken = Get-BearerTokenIDS -endpointUri "$endpoint"
    $headers = @{
        "Authorization" = "$authToken"
        'Content-Type'  = 'application/json'
    }
  } catch {
    Write-LoggerIDS -type "warning" -message "exceptionMessage: $($_.Exception.Message)"
    Write-LoggerIDS -type "warning" -message "errorDetailsMessage: $($_.ErrorDetails.Message)"
  }
  ...
  foreach ($type in $supportedTypes) {
    ...
    if ($null -eq $principalId) {
      ...
      try {
        $principal = Invoke-AzureApiIDS -method GET -uri $uri -headers $headers -rawOutput
  ...
  #Write-LoggerIDS -type "warning" -message "$principalName - $principalId"
  return $principal
}

function Invoke-AzureApiIDS {
  param (
  ...
  if (-Not $headers) {
    $headers = @{
      "Authorization" = "Bearer $(Get-CachedTokenIDS)"
      'Content-Type'  = 'application/json'
    }
  }
  ...
  if ($method -in ("Post", "Put", "Patch", "Delete")) {
    ...
    while ($true) {
      try {
        ...
        Write-LoggerIDS -type "info" -message "statusCode: [ $($response.statusCode) ]"
        # Write-LoggerIDS -type "info" -message "provisioningState: $($responseContent.properties.provisioningState)"
        # Write-LoggerIDS -type "info" -message "provisioningState: $($response.properties.provisioningState)"
        ...
        if ([int32]$_.Exception.Response.StatusCode -like "4*" -or $i -eq $numberOfRetries ) {
          Write-LoggerIDS -type "error" -message "exceptionMessage: $($_.Exception.Message)" -NoFailWhenError
          Write-LoggerIDS -type "error" -message "errorDetailsMessage $($_.ErrorDetails.Message)" -NoFailWhenError
          ...
          $i++
          Write-LoggerIDS -type "warning" -message "exceptionMessage: $($_.Exception.Message)" -NoFailWhenError
          Write-LoggerIDS -type "warning" -message "errorDetailsMessage: $($_.ErrorDetails.Message)"
          Write-LoggerIDS -type "warning" -message "Retrying in [ $secondsBetweenRetries ] seconds, [ $i/$numberOfRetries ]"
...
function Register-ResourceProviderIDS {
  <#
  ...
  $response = Invoke-AzureApiIDS -method GET -uri $uri
  $content = $response.content | ConvertFrom-Json
  if ($content.namespace) {
    Write-LoggerIDS -type "info" -message "Resource provider [$providerNamespace] is already set, nothing to do"
  } else {
    Write-LoggerIDS -type "info" -message "Registering resource provider [$providerNamespace]"
  }
}

function Show-MessageIDS {
...
function Test-NameAvailabilityIDS {
	param (
		[Parameter(Mandatory = $true)][string]$resourceName,
	...
	$results = Invoke-AzureApiIDS -method POST -uri $uri -body $body
	return $results
}

function Test-ResourceIdExistenceIDS {
  param (
    [Parameter(Mandatory = $true)][string]$resourceId
  )

  # the API call to check if the resourceId exists
  Write-LoggerIDS -type "info" -message "Checking whether resource exists or not"
  ...
  try {
    ...
    $resourceProviders = Invoke-AzureApiIDS -method GET -uri $providerUri -throwError
    ...
    $response = Invoke-AzureApiIDS -Method GET -uri $resourceUri -throwError
    if ($response) {
      $idExists = $true
    }
  } catch {
    Write-LoggerIDS -type "debug" -message "ErrorDetails.Message: $($_.ErrorDetails.Message)"
    Write-LoggerIDS -type "debug" -message "Exception.Message: $($_.Exception.Message)"
    if ($_.Exception.Response.StatusCode -eq 404) {
      ...
      $idExists = $false
      Write-LoggerIDS -type "debug" -message "Resource does not exist, the resourceExists is [ $idExists ]. This is not a failure, just a check to determine whether this is a new deployment or an update of an existing one"
    } else {
      Write-LoggerIDS -type "error" -message "Failed to perform a REST API call to check whether the resource exists or not, got error code [ $($_.Exception.Response.StatusCode ) ]" -NoFailWhenError
    }
  }
  return $idExists
}

function Test-SkuAvailabilityIDS {
	Param(
    ...
    $skus = Invoke-AzureApiIDS -uri $url -method "GET"
...
function Write-LoggerIDS {
    ...
    # colorize messages in the ADO pipeline for readability
    if ($type -eq "error") {
